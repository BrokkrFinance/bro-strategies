...
library SwapServiceLib {
    ...
    function swapExactTokensForTokensExt(
        ...
        if (
            ...
            amountOut = swapExactTokensForTokens(swapService, amountIn, minAmountOut, path, new uint256[](0));
        } else {
    ...
    function swapExactTokensForTokens(
        ...
        if (
            ...
            IUniswapV2LikeRouter uniswapV2LikeRouter = IUniswapV2LikeRouter(swapService.router);

            IERC20Upgradeable(path[0]).approve(address(uniswapV2LikeRouter), amountIn);
            ...
            ITraderJoeLBRouter traderjoeLBRouter = ITraderJoeLBRouter(swapService.router);

            IERC20Upgradeable(path[0]).approve(address(traderjoeLBRouter), amountIn);
    ...
    function swapTokensForExactTokens(
        ...
        if (
            ...
            uint256[] memory maxAmountInCalculated = uniswapV2LikeRouter.getAmountsIn(amountOut, path);

            uint256 maxAmountInFinal = Math.min(maxAmountInCalculated[0], maxAmountIn);
            ...
            IERC20Upgradeable(path[0]).approve(address(traderjoeLBRouter), maxAmountIn);
    ...
    function swapExactTokensForNative(
        ...
        if (swapService.provider == SwapServiceProvider.UniswapV2) {
            ...
            IUniswapV2LikeRouter router = IUniswapV2LikeRouter(swapService.router);
    ...
    function swapTokensForExactNative(
        ...
        if (swapService.provider == SwapServiceProvider.UniswapV2) {
            ...
            uint256[] memory maxAmountInCalculated = router.getAmountsIn(amountOut, pathNew);

            uint256 maxAmountInFinal = Math.min(maxAmountInCalculated[0], maxAmountIn);
            IERC20Upgradeable(pathNew[0]).approve(address(router), maxAmountInFinal);
