...
abstract contract StrategyRoleableBaseUpgradeable is RoleableUpgradeable, StrategyBaseUpgradeable {
    uint256[8] private __gap;

    // solhint-disable-next-line
    function __StrategyRoleableBaseUpgradeable_init(StrategyArgs calldata strategyArgs) internal onlyInitializing {
    ...
    function setPriceOracle(IPriceOracle priceOracle) public virtual onlyRole(GOVERNOR_ROLE) {
        super._setPriceOracle(priceOracle);
    }

    function setSwapService(SwapServiceProvider provider, address router) public virtual onlyRole(GOVERNOR_ROLE) {
