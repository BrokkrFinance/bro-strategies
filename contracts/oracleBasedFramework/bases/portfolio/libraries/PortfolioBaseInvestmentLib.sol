...
library PortfolioBaseInvestmentLib {
    ...
    function deposit(
        ...
        if (depositArgs.depositTokenAmountIn == 0) revert IInvestable.ZeroAmountDeposited();
        if (depositArgs.investmentTokenReceiver == address(0)) revert IInvestable.ZeroInvestmentTokenReceiver();

        // check investment limits
        // the underlying defi protocols might take fees, but for limit check we can safely ignore it
        uint256 equityValuationBeforeInvestment = PortfolioBaseAumLib.getEquityValuation(true, false, investableDescs);
        uint256 userEquity;
        uint256 investmentTokenSupply = depositArgs.investmentToken.totalSupply();
        if (investmentTokenSupply != 0) {
            uint256 investmentTokenBalance = depositArgs.investmentToken.balanceOf(depositArgs.investmentTokenReceiver);
        ...
        for (uint256 i = 0; i < investableDescs.length; i++) {
            uint256 embeddedAmount = (depositArgs.depositTokenAmountIn * investableDescs[i].allocationPercentage) /
                Math.SHORT_FIXED_DECIMAL_FACTOR /
                100;
            if (embeddedAmount == 0) continue;
            depositArgs.depositToken.approve(address(investableDescs[i].investable), embeddedAmount);
            investableDescs[i].investable.deposit(embeddedAmount, 0, address(this), params);
        }

        // calculating the actual amount invested into the defi protocol
        uint256 equityValuationAfterInvestment = PortfolioBaseAumLib.getEquityValuation(true, false, investableDescs);
        uint256 actualInvested = equityValuationAfterInvestment - equityValuationBeforeInvestment;
        ...
        depositArgs.investmentToken.mint(
            ...
            InvestableLib.calculateMintAmount(equityValuationBeforeInvestment, actualInvested, investmentTokenSupply)
        );
    }
    ...
    function withdraw(
        ...
        if (withdrawArgs.investmentTokenAmountIn == 0) revert IInvestable.ZeroAmountWithdrawn();
        if (withdrawArgs.depositTokenReceiver == address(0)) revert IInvestable.ZeroDepositTokenReceiver();

        // transferring investment tokens from the caller
        uint256 withdrawnDepositTokenAmount = withdrawArgs.depositToken.balanceOf(address(this));
        // withdrawing from underlying investables
        uint256 investmentTokenSupply = withdrawArgs.investmentToken.totalSupply();
        for (uint256 i = 0; i < investableDescs.length; i++) {
            IInvestable embeddedInvestable = investableDescs[i].investable;
            uint256 embeddedTokenAmountToBurn = (embeddedInvestable.getInvestmentTokenBalanceOf(address(this)) *
                withdrawArgs.investmentTokenAmountIn) / investmentTokenSupply;
            if (embeddedTokenAmountToBurn == 0) continue;
            embeddedInvestable.getInvestmentToken().approve(address(embeddedInvestable), embeddedTokenAmountToBurn);
            embeddedInvestable.withdraw(embeddedTokenAmountToBurn, 0, address(this), params);
        }
        // checking whether enough deposit token was withdrawn
        withdrawnDepositTokenAmount = withdrawArgs.depositToken.balanceOf(address(this)) - withdrawnDepositTokenAmount;
        if (withdrawnDepositTokenAmount < withdrawArgs.minimumDepositTokenAmountOut)
            revert IInvestable.TooSmallDepositTokenAmountOut();
        // burning investment tokens
        withdrawArgs.investmentToken.burnFrom(withdrawArgs.msgSender, withdrawArgs.investmentTokenAmountIn);

        if (withdrawArgs.shouldTransferDepositTokens) {
            //transferring deposit tokens to the depositTokenReceiver
            withdrawArgs.depositToken.safeTransfer(withdrawArgs.depositTokenReceiver, withdrawnDepositTokenAmount);
        }
    }
}
