...
contract TraderJoe is UUPSUpgradeable, StrategyOwnablePausableBaseUpgradeable {
    ...
    string public constant trackingName = "brokkr.traderjoe_strategy.traderjoe_strategy_v1.2.1";
    // solhint-disable-next-line const-name-snakecase
    ...
    function initialize(StrategyArgs calldata strategyArgs, TraderJoeArgs calldata traderJoeArgs) external initializer {
        __checkBinIdsAndAllocations(traderJoeArgs.binIds, traderJoeArgs.binAllocations);

        __UUPSUpgradeable_init();
        __StrategyOwnablePausableBaseUpgradeable_init(strategyArgs);

        __initialize(traderJoeArgs, SwapService(strategyArgs.swapServiceProvider, strategyArgs.swapServiceRouter));
    }

    function reinitialize(TraderJoeArgs calldata traderJoeArgs) external reinitializer(2) {
        __checkBinIdsAndAllocations(traderJoeArgs.binIds, traderJoeArgs.binAllocations);

        TraderJoeStorage storage strategyStorage = TraderJoeStorageLib.getStorage();

        // Initialization.
        __initialize(traderJoeArgs, SwapService(SwapServiceProvider.TraderJoeV2, address(traderJoeArgs.lbRouter)));
        ...
        uint256 lpBalanceToWithdraw = strategyStorage.masterChef.userInfo(strategyStorage.farmId, address(this)).amount;

        strategyStorage.masterChef.withdraw(strategyStorage.farmId, lpBalanceToWithdraw);

        strategyStorage.lpToken.approve(address(strategyStorage.router), lpBalanceToWithdraw);
        ...
        TraderJoeInvestmentLib.deposit(depositTokenIncrement, pairDepositTokenIncrement);
    ...
    function _withdraw(uint256 amount, NameValuePair[] calldata) internal virtual override {
        ...
        TraderJoeInvestmentLib.swapTokens(pairDepositTokenIncrement, strategyStorage.pairDepositToken, depositToken);
    }
    ...
    function _getAssetBalances() internal view virtual override returns (Balance[] memory assetBalances) {
        ...
        for (uint256 i; i < binsAmount; ++i) {
            uint256 balance = strategyStorage.lbPair.balanceOf(address(this), strategyStorage.binIds[i]);

            assetBalances[i] = Balance(address(strategyStorage.lbPair), balance);
        }
    }
        ...
        returns (Valuation[] memory assetValuations)
    {
        ...
        for (uint256 i; i < binsAmount; ++i) {
            ...
            (uint256 reserveX, uint256 reserveY) = strategyStorage.lbPair.getBin(uint24(strategyStorage.binIds[i]));

            uint256 totalSupply = strategyStorage.lbPair.totalSupply(strategyStorage.binIds[i]);

            uint256 valuation;

            if (totalSupply != 0) {
                // Assume that token X and token Y have the same price.
                valuation = (lpTokenBalance * (reserveX + reserveY)) / totalSupply;
            }

            assetValuations[i] = Valuation(address(strategyStorage.lbPair), valuation);
        }
    }

    function _getLiabilityValuations(bool, bool) internal view virtual override returns (Valuation[] memory) {}

    function setSwapService(SwapServiceProvider provider, address router) public virtual override onlyOwner {
    ...
    function adjustBins(uint256[] calldata binIds, uint256[] calldata binAllocations) public onlyOwner {
        __checkBinIdsAndAllocations(binIds, binAllocations);

        TraderJoeInvestmentLib.adjustBins(binIds, binAllocations, getInvestmentTokenSupply());
    }
    ...
    function __checkBinIdsAndAllocations(uint256[] calldata binIds, uint256[] calldata binAllocations) private pure {
    ...
    function __initialize(TraderJoeArgs calldata traderJoeArgs, SwapService memory swapService) private {
        TraderJoeStorage storage strategyStorage = TraderJoeStorageLib.getStorage();

        strategyStorage.lbPair = traderJoeArgs.lbPair;
        strategyStorage.lbRouter = traderJoeArgs.lbRouter;
        strategyStorage.tokenX = IERC20Upgradeable(traderJoeArgs.lbPair.tokenX());
        strategyStorage.tokenY = IERC20Upgradeable(traderJoeArgs.lbPair.tokenY());
        ...
        if (strategyStorage.tokenX == depositToken) {
            strategyStorage.depositToken = IERC20Upgradeable(strategyStorage.tokenX);
            strategyStorage.pairDepositToken = IERC20Upgradeable(strategyStorage.tokenY);
        } else if (strategyStorage.tokenY == depositToken) {
            strategyStorage.depositToken = IERC20Upgradeable(strategyStorage.tokenY);
            strategyStorage.pairDepositToken = IERC20Upgradeable(strategyStorage.tokenX);
        } else {
