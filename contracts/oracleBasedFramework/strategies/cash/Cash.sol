...
contract Cash is UUPSUpgradeable, StrategyOwnablePausableBaseUpgradeable {
    ...
    string public constant trackingName = "brokkr.cash_strategy.cash_strategy_v1.1.0";
    // solhint-disable-next-line const-name-snakecase
    ...
    function _beforeWithdraw(uint256, NameValuePair[] calldata) internal virtual override returns (uint256) {
        return 0;
    }

    function _withdraw(uint256 amount, NameValuePair[] calldata) internal virtual override {}

    function _afterWithdraw(uint256, NameValuePair[] calldata) internal virtual override returns (uint256) {
        return 0;
    }

    function _reapReward(NameValuePair[] calldata params) internal virtual override {}

    function processReward(NameValuePair[] calldata, NameValuePair[] calldata) external virtual override nonReentrant {
